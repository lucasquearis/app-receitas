...
function RecipeDetailButton({ type, recipe, recipeID }) {
  ...
  useEffect(() => {
    const verifyIsInProgress = () => {
      ...
      if (inProgressStorage.cocktails[recipeID]) {
        setRecipeStatus('Continue');
        console.log('Atualizou Bebida');
      } else if (inProgressStorage.meals[recipeID]) {
        setRecipeStatus('Continue');
        console.log('Atualizou Comida', recipeID);
      } else if (doneStorage !== null) {
        const recipeIsDone = doneStorage.find((item) => item.id === recipeID);
        verifyItsDone(recipeIsDone);
        console.log('Comida Finalizada', recipeID);
      }
    };
    verifyIsInProgress();
  }, [recipeID]);
