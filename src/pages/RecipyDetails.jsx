...
const DataManeger = (data, receita) => {
  if (receita === "comidas" && data) {
    const { meals } = data;
    const ingredients = Object.entries(meals[0]).filter(n => {return n[0].indexOf("strIngredient") !== -1 });
    const measures = Object.entries(meals[0]).filter(n => {return n[0].indexOf("strMeasure") !== -1 });
    const obj = {
  ...
  const { drinks } = data;
  const ingredients = Object.entries(drinks[0]).filter(n => {return n[0].indexOf("strIngredient") !== -1 });
  const measures = Object.entries(drinks[0]).filter(n => {return n[0].indexOf("strMeasure") !== -1 });
  const objdrink = {
...
function RecipyDetails() {
  ...
  return (
    data
    ?     <>
    <img className="imgtittle" data-testid="recipe-photo" src={ data.img } alt="detalhes" />
    ...
    { data && receita === "bebidas" ? <h3 data-testid="recipe-category">{data.type}</h3> : <></>}
    <h2>Ingredientes</h2>

    <div className="ingredients">
              <lo>    
    {
      data.ingredients[0].map((n,index) => (
        n[1] !== "" && n[1] !== null 
        ? <li data-testid={`${index}-ingredient-name-and-measure`} >{n[1]}</li>
        :<></>
        ))
      }
        </lo>
        <lo>
    {
      data.measures[0].map((n,index) => (
        n[1] !== "" && n[1] !== null 
        ? <li className="nodot" data-testid={`${index}-ingredient-name-and-measure`} >{n[1]}</li>
        :<></>
        ))
      }
        </lo>
