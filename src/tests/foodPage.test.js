...
describe('test se há todos os componente', () => {
  ...
  it(`verifica se há 7 botões com as 6 primeiras 
    ...
    global.fetch.mockResolvedValue(mockMealsCategory);
    renderWithRouter(<Provider><Comidas /></Provider>);
  ...
  it(`verifica se há 17 imagens, 
    ...
    jest.spyOn(global, 'fetch');
    global.fetch.mockResolvedValue(mockMealsCategory);
    global.fetch.mockResolvedValue(mockAllMeals);
    renderWithRouter(<Provider><Comidas /></Provider>);
  ...
  it('verifica os textos', async () => {
    ...
    global.fetch.mockResolvedValue(mockAllMeals);
    renderWithRouter(<Provider><Comidas /></Provider>);
    const textComidas = await screen.findByText(/comidas/i);
    const textAll = await screen.findByText(/all/i);
    expect(textComidas.tagName).toBe('H1');
